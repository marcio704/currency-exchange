...
class CurrencyConverterService {
    ...
    def isChangingNotificationSent (currencyFrom, currencyTo, amount) {
    	def newAmount = convert(currencyFrom, currencyTo, amount)
    	def diff = newAmount - CurrencyJob.CURRENT_AMOUNT
    	
    	if(Math.abs(diff) >= CurrencyJob.CURRENT_AMOUNT * CurrencyJob.ALERT_IN/100) {
    		String msg = "Alert! ${currencyFrom} to ${currencyTo} has changed from ${CurrencyJob.CURRENT_AMOUNT} to ${newAmount}. Total ${(diff/CurrencyJob.CURRENT_AMOUNT)*100}%" 
